...
def process_file():
    ...
    if len(output.get()) > 0:
        cmd = [sys.executable, os.path.join(current_working_directory, 'firstocr.py'), '-p', item.get(), '-o', output.get()]
    else:
        cmd = [sys.executable, os.path.join(current_working_directory, 'firstocr.py'), '-p', item.get()]
